...
SELECT
    COUNT(*) AS total_left,
    CONCAT(
        FLOOR(AVG(EXTRACT(YEAR FROM AGE(exit_date, hire_date)))),
        ' years ',
        FLOOR(AVG(EXTRACT(MONTH FROM AGE(exit_date, hire_date)))),
        ' months ',
		 FLOOR(AVG(EXTRACT(DAY FROM AGE(exit_date, hire_date)))),
        ' days'
    ) AS avg_tenure
FROM
    employee
